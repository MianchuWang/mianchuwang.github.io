...
window.QUIZ_DATA["lecture10-advances"] = {
  ...
  questions: [
    // ---------- Roofline & performance math ----------
    {
      ...
      options: [
        "≈ 63 FLOPs/byte — very close to B, since the weight-matrix term DF dominates the bytes",
    ...
    },
    {
      ...
      options: [
        ...
        "989 TFLOP/s — peak is always achievable with good kernels",
    ...
    },
    {
      ...
      options: [
        "≈ 0.3% — about 130 tokens/s × 2.6e10 FLOPs/token ≈ 3.4 TFLOP/s out of 989",
    ...
    },
    {
      ...
      options: [
        ...
        "B ≈ 295 — quantization doesn't change the crossover",
    ...
    },
    {
      ...
      options: [
        "≈ 8 (i.e., ≈ K, since S ≫ K) — verification raises attention intensity by K×, another reason speculation is nearly free",
    ...
    },
    {
      ...
      options: [
        "Latency stays roughly flat (same weight bytes per step) while throughput grows ≈ linearly in B — until the crossover, after which latency grows with B and throughput plateaus",
        "Latency grows linearly and throughput stays flat at all batch sizes",
    ...
    },
    {
      ...
      options: [
        "L = 50 concurrent requests — each needs a KV cache allocation, so average concurrency × per-request KV determines the memory the server must provision",
    ...
    },
    {
      ...
      options: [
        "(a) 8× smaller; (b) ≈ 8× higher — each loaded KV byte now serves 8 query heads' worth of attention FLOPs",
    ...
    },
    {
      ...
      options: [
        "The attention score qᵀk = (W_Q h)ᵀ(W_UK c) = hᵀ(W_Qᵀ W_UK)c, so the query and key up-projections collapse into one precomputed matrix applied between the hidden state and the cached latent c",
        "Softmax is linear, so it can be applied directly to the latent",
    ...
    },
    {
      ...
      options: [
        "≈ 16× — the techniques act on independent factors of the cache formula (heads × layers), so their savings multiply",
        "≈ 10× — savings add (8 + 2)",
        "≈ 8× — CLA is subsumed by GQA",
        "≈ 4× — combining them causes interference"
      ],
      correct: [0],
    ...
    },
    {
      ...
      options: [
        ...
        "≈ 6× — one global layer per six",
        "≈ 8× — the window is 8× smaller than the context",
        "≈ 1.2× — local layers barely help"
      ],
      correct: [0],
    ...
    },
    {
      ...
      options: [
        "Attention 'sinks': softmax must distribute probability mass somewhere, and models learn to park excess mass on the initial tokens — evicting them distorts every downstream attention distribution",
    ...
    },
    {
      ...
      options: [
        "KV memory per sequence is capped at W entries per layer regardless of how long generation runs — old entries are overwritten in place, with no eviction bookkeeping",
    ...
    },
    {
      ...
      options: [
        "1 − TV(p, q), where TV is the total variation distance — acceptance is high exactly when draft and target distributions are close",
    ...
    },
    {
      ...
      options: [
        "The probability that position i is even reached decays as αⁱ, while drafting cost grows linearly in K — late draft tokens are usually thrown away but always paid for",
    ...
    },
    {
      ...
      options: [
        "Accept a draft token iff it equals the target's argmax; on mismatch, emit the target's argmax — the output is exactly the target's greedy sequence",
        "Accept every draft token, since greedy sampling has no randomness",
        "It cannot be used at temperature 0 — rejection sampling needs randomness",
    ...
    },
    {
      ...
      options: [
        "Autoregress in FEATURE space: a small head predicts the target's next top-layer hidden state from the current one (plus the sampled token), then reuses the target's own output embedding to get token distributions — capturing sequential dependence that Medusa's parallel heads miss",
    ...
    },
    {
      ...
      options: [
        "Accepted tokens already contribute probability min(p, q); only the residual tops each token up to exactly p(x). Resampling from p would over-weight tokens where q was already adequate, giving min(p,q) + TV·p ≠ p",
        "Sampling from p would require an extra forward pass of the target model",
    ...
    // ---------- Quantization, deeper ----------
    {
      ...
      options: [
        "Layer-by-layer weight quantization that minimizes the output reconstruction error ‖WX − ŴX‖² on calibration data, quantizing weights sequentially while compensating remaining weights using (approximate) second-order/Hessian information",
    ...
    },
    {
      ...
      options: [
        "E4M3 — forward-pass values have modest dynamic range, so spending bits on mantissa (precision) beats spending them on exponent (range); E5M2's extra range mainly matters for gradients in training",
        "E5M2 — inference always needs maximum dynamic range",
    ...
    },
    {
      ...
      options: [
        "W4A16 excels at small-batch memory-bound decode (fewest weight bytes) but leaves compute in bf16; W8A8 also accelerates the compute-bound regime (prefill, large batches) because int8 tensor cores double throughput",
        "W8A8 is strictly better in all regimes",
        "W4A16 accelerates prefill more than decode",
    ...
    },
    {
      ...
      options: [
        "Going bf16 → int8/fp8 halves KV memory, roughly doubling the batch size that fits — and thus throughput, when KV-capacity-limited",
        "It reduces the per-step KV bandwidth cost, which dominates at long contexts and large batches",
        "Keys are typically harder to quantize than values, due to outlier channels (and RoPE-induced structure), motivating per-channel key scales",
    ...
    },
    {
      ...
      options: [
        "At 4 bits there are only 16 levels, so a scale must track local weight statistics closely to avoid crushing precision; groups of ~128 adapt to local ranges while adding only ~0.1 bits/weight of overhead and staying kernel-friendly",
        "Per-tensor scales are more accurate but too slow to compute",
        "Group-wise scaling is required by the CUDA compiler",
        "It allows different groups to use different bit-widths at runtime"
      ],
      correct: [0],
    ...
    },
    {
      ...
      options: [
        "Activations are input-dependent — ranges vary per token and per prompt, must be scaled at runtime, and exhibit large systematic outlier channels — whereas weights are static, known offline, and can be optimized with unlimited calibration effort",
    ...
    // ---------- Systems, deeper ----------
    {
      ...
      options: [
        "It prevents a long prefill from monopolizing the GPU and stalling all running decodes (inter-token latency spikes); the trade is slightly higher TTFT for the prefilling request and some efficiency loss on the split matmuls",
    ...
    },
    {
      ...
      options: [
        "Prefill (compute-bound, TTFT-sensitive) and decode (memory-bound, ITL-sensitive) have conflicting optimal batch sizes, parallelism strategies, and scaling needs; separating them removes interference and lets each pool be provisioned to its own SLO — if the KV transfer over the interconnect is cheap enough",
    ...
    },
    {
      ...
      options: [
        "A decode step launches hundreds of tiny kernels, each finishing in microseconds at small batch — comparable to its CPU launch overhead — so the GPU starves waiting on the CPU; graph replay removes per-kernel launch cost. Prefill's big kernels amortize launches naturally",
    ...
    },
    {
      ...
      options: [
        "Decode has ONE query per sequence, so parallelism is only batch × heads — far too few thread blocks to fill a GPU at long context; FlashDecoding additionally splits the KV sequence into chunks processed by parallel blocks, then merges partial softmax results with a log-sum-exp rescaling reduction",
    ...
    },
    {
      ...
      options: [
        "Cross-request, cross-time prefix reuse: a new request whose prompt shares a prefix with any cached earlier request (same system prompt, few-shot template, chat history) skips prefill for the shared part — an exact optimization managed with LRU eviction",
    ...
    },
    {
      ...
      options: [
        "For short-to-moderate contexts: recompute is one compute-bound prefill pass on data already at hand, often faster than paging gigabytes back over PCIe; swapping wins for very long contexts where re-prefill cost exceeds the transfer",
        "Never — recomputation gives different results than the saved cache",
    ...
    },
    {
      ...
      options: [
        "Small blocks minimize internal fragmentation (≤1 partially-empty block per sequence) but inflate the block-table/indirection overhead and hurt kernel memory-access efficiency; large blocks read/write contiguously but waste more reserved-yet-unused slots",
        "Larger blocks improve model accuracy by giving attention more context",
    ...
    },
    {
      ...
      options: [
        "TP shards every weight matrix, so each GPU streams ~1/8 of the bytes per step in parallel — per-token latency drops up to ~8× (minus all-reduce costs); PP still passes each token through every layer serially, so per-token latency stays the same (PP only raises throughput via pipelining)",
        "TP caches the KV on all GPUs redundantly, which is faster",
        "PP is always better for decode because it needs no communication",
    ...
    },
    {
      ...
      options: [
        "Floating-point addition is non-associative, and continuous batching changes which requests are batched together — different batch shapes select different kernels/reduction orders, producing tiny logit differences that occasionally flip an argmax and then diverge",
    ...
    // ---------- MoE & alternative architectures, deeper ----------
    {
      ...
      options: [
        "At B=1 only the ~2 routed experts per layer (plus attention weights) are read — traffic comparable to a ~13B dense model; as B grows, different tokens route to different experts, so traffic climbs toward reading ALL experts, eroding MoE's bandwidth advantage until amortization takes over",
    ...
    },
    {
      ...
      options: [
        "Its SSM parameters (input/forget dynamics) became functions of the current input — 'selectivity' — letting the model decide per token what to write into or erase from its fixed-size state, rather than applying the same time-invariant filter to everything",
    ...
    },
    {
      ...
      options: [
        "A few attention layers restore capabilities fixed-size states handle poorly — exact long-range retrieval, copying, and strong in-context learning — while 7/8 of layers still avoid KV cache growth, keeping most of the inference advantage",
    ...
    },
    {
      ...
      options: [
        "Linear attention's fixed-size state gives cheap but 'fuzzy' global memory that can't do precise recall; the exact local window handles sharp recent-token retrieval that fuzzy states miss — together they approximate full attention's recall at O(1) state",
        "Linear attention handles short contexts and local attention handles long ones",
        "Local attention corrects the numerical instability of linear attention's kernel",
    ...
    },
    {
      ...
      options: [
        "When n ≪ T and there is spare COMPUTE: each diffusion pass is a compute-bound full-sequence forward (like prefill, no incremental caching), while AR steps are memory-bound; diffusion trades many cheap steps for few expensive ones — a good trade at low batch where compute idles, poor at high batch where compute is already saturated",
    ...
    },
    {
      ...
      options: [
        "Transformer KV grows linearly in S (e.g., ~0.8 MB/token → 80 GB at 100k tokens for a 13B-class MHA model), while the SSM state is a fixed few-hundred-MB regardless of S — so SSMs' advantage widens without bound as contexts lengthen, at the cost of lossy history compression",
        "Both grow linearly, but the SSM constant is smaller",
        "The SSM state grows with S while the KV cache is fixed",
    ...
    // ---------- Serving metrics & scenario ----------
    {
      ...
      options: [
        "Latency distributions in LLM serving are heavy-tailed — queueing, prefill interference, and preemptions create rare but severe stalls that the mean hides; users experience the tail, and a system can raise raw throughput while goodput (SLO-compliant work) actually falls",
    ...
    },
    {
      ...
      options: [
        "Prefix caching (radix-tree KV reuse) — the shared 10k-token repository context is prefilled once, slashing TTFT for every subsequent request",
        "Chunked prefill or prefill/decode disaggregation — prevents the long prefills from spiking other users' inter-token latency",
        "Speculative decoding — accelerates the memory-bound decode phase for smooth, fast token streaming",
