...
2. **Where the learning goes** — probe once at step 0: sample 8 answers per val prompt with the base model and freeze each prompt's solve rate as its difficulty label — hard (0–20%), mid (20–80%), easy (80–100%). Then plot val accuracy per bucket, alongside the share of total $|\hat{A}|$ mass contributed by near-unanimous groups ($k \le 1$ or $k \ge G-1$): $\sum_{i \in \text{near-unan.}} |\hat{A}_i| \,/\, \sum_{i \in \text{batch}} |\hat{A}_i|$, one $\hat{A}_i$ per rollout (a batch is $64 \times G = 768$). All-wrong and all-correct groups have $\hat{A} \equiv 0$, so the share effectively measures lone-success and lone-failure groups — where $/\,\mathrm{std}$ amplifies most (at $k=1$, std $\approx 0.28$, a $3.5\times$ boost).
