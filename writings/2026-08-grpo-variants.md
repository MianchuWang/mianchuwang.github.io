...
- **$1/|o_i|$ gone** — when $\hat{A}_i < 0$, it pushed each token of a *long* failure down more weakly than a short one's, so short failure modes die first and the surviving wrong answers skew long (prediction: wrong-answer length grows under GRPO, not here).
