...
> so lengthening every response just scales a mean-centered sum: a two-response group with $\hat{A} = \pm 1$ and length $L$ gives $\frac{1}{2}(L - L) = 0$ for any $L$. Nor can the policy "grow" its winners: $|o_i|$ is sampled data, not a decision variable, and the winner's EOS token sits inside $\sum_t$ — the gradient reinforces its decision to stop exactly where it did ($\sum_t \nabla\log\pi_\theta(o_{i,t}) = \nabla\log\pi_\theta(o_i)$, the unbiased score of the whole sequence). The normalizers — $G$ for Dr. GRPO, $G\,\overline{|o|}$ for DAPO — therefore change only the **magnitude and variance** of the gradient (verbose batches take bigger, noisier steps under Dr. GRPO, stable ones under DAPO), never its **direction**. The one *directional* length bias in this family is GRPO's $1/|o_i|$, which shrinks long sequences' weight asymmetrically.
